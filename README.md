...
1. A token on your own GPUs costs cents in electricity; a bought token costs dollars.
   That gap is a **glacier of savings** — and it is **melting**, because API prices halve
   roughly yearly (the sun) while electricity drifts up (the warm ground).
...
4. Electricity going up is a real **downward force vector on token yield** — worth
   0.2–0.5%/yr of drag — but API prices melt at ~70%/yr. The meter rate nibbles;
   the *megawatts* veto.
...
- Electricity floor: **$0.17 per million open tokens** — power is ~4–9% of build cost;
  chips and people are the money.
- Melt time vs the frontier API: **6.4 years**; vs hosted open-weights: **2.1 years**.
- Minimum viable scale vs the frontier: **≈ 18B movable tokens/month**. Below that, buy.
- Even at 50B tokens/month, **hosted open-weights beat building by ~$3.3M over 3 years**
  at typical utilization — self-hosting wins outright only with provider-grade
  utilization or when data can't leave.

*Analytical work, not investment advice. Defaults are knowledge-based estimates as of
August 2026 — the dashboard exists so you can disagree with them.*
